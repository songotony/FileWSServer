...
function authUserPullRequest(socket, json, server) {
	...
	console.log("Client " + json.username + " : Pre-authentifying on pullrequest");
	if (server == undefined)
		return;
...
function getPullRequest(socket, json) {
	...
	if (json == undefined || json.id == undefined || json.id == "")
	...
	json.project = socket.file.project.name;
	json.recursively = true;
	file.get(socket, json).then(function() {
		if (socket.readyState === ws.OPEN)
			socket.send(JSON.stringify(new responses.OK("pullRequestGet", "OK")));
		console.log("Client " + socket.file.username + " : Successfully retrieved pull request '" + json.id + "'");
	});
}
...
function createPullRequest(socket, json) {
	...
	if (json == undefined || json.title == undefined || json.title == "" || json.description == undefined)
	...
	fs.mkdirSync(path.join(config.baseDir, socket.file.username, socket.file.project.name, dest));
...
function finishPullRequest(socket, json, server) {
	...
	for (let client of server.clients)
		if (socket !== client && client.file.username == socket.file.username && client.file.project.name == socket.file.project.name) {
			...
			if (client.readyState === ws.OPEN)
				client.send(JSON.stringify(new responses.OK("pullRequestCreated", {id : socket.pr.actual.dir, owner : socket.pr.username, title : socket.pr.actual.title, description : socket.pr.actual.description, date : socket.pr.actual.date, files : socket.pr.actual.files })));
		}
	if (socket.readyState === ws.OPEN)
	...
	socket.pr.actual.files = [];
}

function deletePullRequest(socket, json) {
	var i = 0;

	console.log("Client " + socket.file.username + " : Deleting pullrequest");
	if (!auth.verify(socket, config.key))
		return;
	if (json == undefined || json.id == undefined || json.id == "")
		throw new exceptions.ParametersException("Missing delete pullrequest parameters");
	for (; i < socket.file.project.pullrequest.length; i++)
		if (socket.file.project.pullrequest[i] == json.id)
			break;
	if (i == socket.file.project.pullrequest.length)
		throw new exceptions.ParametersException("Pullrequest ID does not exist");
	rimraf(path.join(config.baseDir, socket.file.username, socket.file.project.name, json.id), {disableGlob : true}, function(err) {
		if (err)
			return console.error(err);
		console.log("Pullrequest id " + json.id + " successfully cleaned");
	});
}
...
module.exports = {
	...
	funcs : [
		...
		{subtype : 'delete', func : deletePullRequest}
	]
}
